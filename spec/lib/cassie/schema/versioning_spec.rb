module Cassie::Schema
  RSpec.describe Cassie::Schema::Versioning do
    ...
    describe "record_version" do
      ...
      it "inserts version into cassandra" do
        expect_any_instance_of(InsertVersionQuery).to receive(:execute!) do |query|
          expect(query.version).to eq(version)
        end
        mod.record_version(version)
      end
    end
    ...
    describe "next_version" do
      context "when local_versions are empty" do
        ...
        it "gives 0.0.1.0 version" do
          expect(mod.next_version).to eq(Version.new('0.0.1.0'))
        end
      end
