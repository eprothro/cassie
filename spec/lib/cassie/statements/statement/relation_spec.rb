RSpec.describe Cassie::Statements::Statement::Relation do
  let(:execution) do
    double(id: argument, if_meth: false, term_meth: "now()")
  end
  let(:argument){ double }
  let(:klass){ Cassie::Statements::Statement::Relation }
  let(:args){ [identifier, operation, value_method, opts] }
  let(:identifier){ :id }
  let(:operation){ :eq }
  let(:value_method){ :id }
  let(:opts){ {} }
  ...
  describe "#to_cql" do
    it "is relation syntax with positional placeholder" do
      expect(object.to_cql).to eq("id = ?")
    end
    context "when disabled" do
      let(:opts){ {if: false} }
      it "is nil" do
        expect(object.to_cql).to be_nil
      end
    end
  end
